...
# Create an inner frame for the label
...
# Function to dynamically adjust button size based on label content
def update_button_size(button):
    width = len(label.cget("text")) * 10  # Adjust the multiplier based on your preference
    button.configure(width=width)  # Use 'configure' instead of 'config'
